...
$based = ("SELECT id_representante,nombre_representante FROM representante");
$rs = mysql_query($based);
$opciones = "<option value='seleccione representante'>Seleccione representante</option>";
...
if (mysql_num_rows($rs) > 0) {
    while ($fila = mysql_fetch_assoc($rs)) {
        $opciones .= "<option value='". $fila["id_representante"]."'>". $fila["nombre_representante"]."</option>";
    }
}
echo $opciones;
?>