...
$based = ("SELECT * FROM estados");
$rs = mysql_query($based);
$opciones = "<option value='seleccione estado'>Seleccione Estado</option>";
...
if (mysql_num_rows($rs) > 0) {
    while ($fila = mysql_fetch_assoc($rs)) {
        $opciones .= "<option value='". $fila["id_estado"]."'>". $fila["nombre_estado"]."</option>";
    }
}
echo $opciones;
?>